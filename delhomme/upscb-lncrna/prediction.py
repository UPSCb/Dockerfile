...
os.system("python " + os.path.dirname(os.path.realpath(__file__)) + "/bin/extractfeatures.py "+in_file+" 1")
...
if no_ff_flag==True:
	...
	os.system(os.path.dirname(os.path.realpath(__file__)) + "/lib/framefinder/framefinder -r False -w " + os.path.dirname(os.path.realpath(__file__)) + "/lib/framefinder/framefinder.model "+in_file+" > "+in_file+"_ffout")
	#parse framefinder results and write framefinder feature files
	if vflag:
		print 'Extracting Framefinder Features...'
	os.system("python " + os.path.dirname(os.path.realpath(__file__)) + "/bin/ffparse.py "+in_file+"_ffout")
...
if noblast_flag==False:
	if blastres_flag==True:
		...
		os.system("python " + os.path.dirname(os.path.realpath(__file__)) + "/bin/blastparse_mt3.py "+blastres_file)
		if vflag:
			print 'Merging all Features...'
		os.system("python " + os.path.dirname(os.path.realpath(__file__)) + "/bin/mergefeatures.py "+in_file+"_features "+no_ff_flagval+" "+ff_featurefile_pos+" "+noblast_flag_val+" "+blastres_file+"_blastfeatures "+" "+in_file+"_all_features")
	else:
		...
		#$bcommand=os.path.dirname(os.path.realpath(__file__)) + "/lib/blast/bin/blastx -query "+in_file+" -db "+blastdb+" -outfmt '6 qseqid sseqid pident evalue qcovs qcovhsp score bitscore qframe sframe' -out "+blastres_pos+" -qcov_hsp_perc "+ str(max_qcov_hsp)+" -num_threads "+str(num_threads)
                bcommand="diamond blastx -q "+in_file+" -d "+blastdb+" -f 6 qseqid sseqid pident evalue qcovhsp score bitscore qframe -o "+blastres_pos+" -p "+str(num_threads)
		print str(bcommand)
		os.system(str(bcommand))
		if vflag:
			print 'Parsing Blast Results...'

		os.system("python " + os.path.dirname(os.path.realpath(__file__)) + "/bin/blastparse_mt3.py "+blastres_pos)
		print 'Merging all Features...'
		os.system("python " + os.path.dirname(os.path.realpath(__file__)) + "/bin/mergefeatures.py "+in_file+"_features "+no_ff_flagval+" "+ff_featurefile_pos+" "+noblast_flag_val+" "+blastres_pos+"_blastfeatures "+" "+in_file+"_all_features")
...
if vflag:
	print 'Predicting...'
print str("python " + os.path.dirname(os.path.realpath(__file__)) + "/bin/rf/predict.py "+in_file+"_all_features "+model_file+" "+out_file+" "+lflag_val+" "+label_file)
os.system("python " + os.path.dirname(os.path.realpath(__file__)) + "/bin/rf/predict.py "+in_file+"_all_features "+model_file+" "+out_file+" "+lflag_val+" "+label_file)
