...
EDITED to read diamond blastx instead
diamond blastx -q "+in_file+" -d "+blastdb+" -outfmt '6 qseqid sseqid pident evalue qcovhsp score bitscore qframe' -o "+blastres_pos+" -p "+str(num_threads)
		...
		for q in self.ids:
			...
			for line in self.data:
				if q == line.split('\t')[0]:
					...
					qframe=int(line.split('\t')[7])
					#print 'q   qframe'
					#print q,str(qframe)
					
					ctr=ctr+1
					hitscore=hitscore+(-1*getlog10(evalue))
					bitscore=bitscore+float(line.split('\t')[6])
					if(qframe==1):
						frame1=frame1+1
					elif(qframe==2):
						frame2=frame2+1
